...
class ExecutionEngine:
    ...
    def run(self, raw_line: str) -> None:
        ...
        if not line or line.startswith('#'):
            if line.startswith('#'): self.formatter.render_comment(line)
            return
        ...
        try:
            ...
            if not self.security.has_access(self.context.current_directory, 
                                            self.context.current_user, 
                                            required):
                raise PermissionError(f"Permission denied: user '{self.context.current_user}' "
